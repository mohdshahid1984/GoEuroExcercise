...
{
    static final Logger logger = LoggerFactory.getLogger(TrainSearchTests.class);
    ...
    public void testSortingOfTrainPrices(SearchTestData data)
    {
        ...
        homePage.openBaseUrl(Configuration.APPLICATION_URL);

        logger.debug("---Thread.currentThread().getId() = "+Thread.currentThread().getId() +"and Driver is = " + DriverManager.getWebDriver().hashCode() + "----Duplicate1");
        homePage.performSearch(data);
        logger.debug("---Thread.currentThread().getId() = "+Thread.currentThread().getId() +"and Driver is = " + DriverManager.getWebDriver().hashCode() + "----Duplicate2");

        SearchResultsPage searchResultsPage = new SearchResultsPage();
        logger.debug("---Thread.currentThread().getId() = "+Thread.currentThread().getId() +"and Driver is = " + DriverManager.getWebDriver().hashCode() + "----Duplicate3");
        ...
        logger.debug("---Thread.currentThread().getId() = "+Thread.currentThread().getId() +"and Driver is = " + DriverManager.getWebDriver().hashCode() + "----Duplicate4");
