...
public class TrainRestAPITests
{
    ...
    public void trainSortingTests(SearchTestData data)
    {
        ...
        travelModes.add("Train");travelModes.add("Flight");travelModes.add("Bus");
        ...
        JSONObject resp = (JSONObject)new RequestSender().sendPOSTRequest("/GoEuroAPI/rest/api/v5/searches", req);

        String searchId = (String)resp.get("searchId");

        System.out.println("==============="+searchId+"====================");
        WaitUtils.delay_N_Sec(2);
        JSONObject searchResp = (JSONObject)new RequestSender().sendGETTRequest("/GoEuroAPI/rest/api/v5/results?travel_mode=train&all_positions=true&include_price_details=true&sort_by=price&sort_variants=price&search_id="+searchId);
